...
function assert4_snat_is_configured() {
	...
	# Handle ipcalc syntax conflict on Debian/Ubuntu
	if [[ $(egrep -q "Debian|Ubuntu" /etc/issue;echo $?) == 0 ]];then
		local ip=$(ipcalc -b ${OCSERV_CLIENT_IPADDR} | grep "Address:" | cut -d':' -f2 | sed 's/\s//g')
		local network=$(ipcalc -b ${OCSERV_CLIENT_IPADDR} | grep "Network:" | cut -d':' -f2 | sed 's/\s//g')
	else
		local ip=$(echo ${OCSERV_CLIENT_IPADDR}|sed 's/\/.*//g')
		# Works on CentOS and Fedora
		local network=$(ipcalc -n ${OCSERV_CLIENT_IPADDR}|cut -d'=' -f2)
	fi
	local has_snat=$(
		iptables -t nat -S POSTROUTING | \
      egrep "MASQUERADE|SNAT" \
		| egrep -q "\-s $ip|\-s $network";echo $?)
		...
		while [[ $# -gt 1 ]]; do
			...
			for route in ${OCSERV_ROUTES4}; do
				...
				local curl_status=$(ip netns exec \
					...
					&> /dev/null; echo $?)
        # Pass for DROP(7) or REJECT(28)
				if [[ ${curl_status}  == "7" || ${curl_status} == "28" ]];then
					echo -e "->  PASS: \
...
function assert4_disallowed_routes_are_not_reachable_on_allowed_ports() {
	echo "Assert disallowed routes are not reachable on allowed ports:"
